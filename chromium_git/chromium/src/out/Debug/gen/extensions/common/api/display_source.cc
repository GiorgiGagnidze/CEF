...
std::string ToString(ErrorType enum_param) {
  switch (enum_param) {
    case ERROR_TYPE_CREATE_MEDIA_PIPELINE_ERROR:
      return "create_media_pipeline_error";
    case ERROR_TYPE_EXCEEDED_SESSION_LIMIT_ERROR:
      return "exceeded_session_limit_error";
    case ERROR_TYPE_ESTABLISH_CONNECTION_ERROR:
      return "establish_connection_error";
    case ERROR_TYPE_CAPABILITIES_NEGOTIATION_ERROR:
      return "capabilities_negotiation_error";
    case ERROR_TYPE_MEDIA_SEND_ERROR:
      return "media_send_error";
    case ERROR_TYPE_CONNECTION_ERROR:
      return "connection_error";
    case ERROR_TYPE_UNEXPECTED_MESSAGE_ERROR:
      return "unexpected_message_error";
...
ErrorType ParseErrorType(const std::string& enum_string) {
  if (enum_string == "create_media_pipeline_error")
    return ERROR_TYPE_CREATE_MEDIA_PIPELINE_ERROR;
  if (enum_string == "exceeded_session_limit_error")
    return ERROR_TYPE_EXCEEDED_SESSION_LIMIT_ERROR;
  if (enum_string == "establish_connection_error")
    return ERROR_TYPE_ESTABLISH_CONNECTION_ERROR;
  if (enum_string == "capabilities_negotiation_error")
    return ERROR_TYPE_CAPABILITIES_NEGOTIATION_ERROR;
  if (enum_string == "media_send_error")
    return ERROR_TYPE_MEDIA_SEND_ERROR;
  if (enum_string == "connection_error")
    return ERROR_TYPE_CONNECTION_ERROR;
  if (enum_string == "unexpected_message_error")
    return ERROR_TYPE_UNEXPECTED_MESSAGE_ERROR;
...
scoped_ptr<base::DictionaryValue> ErrorInfo::ToValue() const {
  ...
  value->SetWithoutPathExpansion("type", new base::StringValue(display_source::ToString(this->type)));
...
std::string ToString(SinkState enum_param) {
  switch (enum_param) {
...
// static
scoped_ptr<SinkInfo> SinkInfo::FromValue(const base::Value& value) {
  scoped_ptr<SinkInfo> out(new SinkInfo());
  if (!Populate(value, out.get()))
    return scoped_ptr<SinkInfo>();
  return out;
}

scoped_ptr<base::DictionaryValue> SinkInfo::ToValue() const {
  scoped_ptr<base::DictionaryValue> value(new base::DictionaryValue());
  ...
  value->SetWithoutPathExpansion("state", new base::StringValue(display_source::ToString(this->state)));
...
std::string ToString(AuthenticationMethod enum_param) {
  switch (enum_param) {
...
// static
scoped_ptr<AuthenticationInfo> AuthenticationInfo::FromValue(const base::Value& value) {
  scoped_ptr<AuthenticationInfo> out(new AuthenticationInfo());
  if (!Populate(value, out.get()))
    return scoped_ptr<AuthenticationInfo>();
  return out;
}

scoped_ptr<base::DictionaryValue> AuthenticationInfo::ToValue() const {
  scoped_ptr<base::DictionaryValue> value(new base::DictionaryValue());

  value->SetWithoutPathExpansion("method", new base::StringValue(display_source::ToString(this->method)));
...
scoped_ptr<base::DictionaryValue> StartSessionInfo::AudioTrack::ToValue() const {
  scoped_ptr<base::DictionaryValue> value(new base::DictionaryValue());
...
scoped_ptr<base::DictionaryValue> StartSessionInfo::VideoTrack::ToValue() const {
  scoped_ptr<base::DictionaryValue> value(new base::DictionaryValue());
  ...
  if (dict->GetWithoutPathExpansion("authenticationInfo", &authentication_info_value)) {
    {
      ...
      if (!authentication_info_value->GetAsDictionary(&dictionary)) {
        ...
        scoped_ptr<AuthenticationInfo> temp(new AuthenticationInfo());
  ...
  if (dict->GetWithoutPathExpansion("audioTrack", &audio_track_value)) {
    {
      ...
      if (!audio_track_value->GetAsDictionary(&dictionary)) {
        ...
        scoped_ptr<AudioTrack> temp(new AudioTrack());
  ...
  if (dict->GetWithoutPathExpansion("videoTrack", &video_track_value)) {
    {
      ...
      if (!video_track_value->GetAsDictionary(&dictionary)) {
        ...
        scoped_ptr<VideoTrack> temp(new VideoTrack());
...
// static
scoped_ptr<StartSessionInfo> StartSessionInfo::FromValue(const base::Value& value) {
  scoped_ptr<StartSessionInfo> out(new StartSessionInfo());
  if (!Populate(value, out.get()))
    return scoped_ptr<StartSessionInfo>();
  return out;
}

scoped_ptr<base::DictionaryValue> StartSessionInfo::ToValue() const {
  scoped_ptr<base::DictionaryValue> value(new base::DictionaryValue());

  value->SetWithoutPathExpansion("sinkId", new base::FundamentalValue(this->sink_id));

  if (this->authentication_info.get()) {
    value->SetWithoutPathExpansion("authenticationInfo", (this->authentication_info)->ToValue().release());

  }
  if (this->audio_track.get()) {
    value->SetWithoutPathExpansion("audioTrack", (this->audio_track)->ToValue().release());

  }
  if (this->video_track.get()) {
    value->SetWithoutPathExpansion("videoTrack", (this->video_track)->ToValue().release());
...
scoped_ptr<base::ListValue> Results::Create(const std::vector<linked_ptr<SinkInfo> >& result) {
...
scoped_ptr<Params> Params::Create(const base::ListValue& args) {
  ...
  if (args.Get(0, &sink_id_value) &&
      !sink_id_value->IsType(base::Value::TYPE_NULL)) {
    {
...
scoped_ptr<base::ListValue> Results::Create(const AuthenticationInfo& result) {
  scoped_ptr<base::ListValue> create_results(new base::ListValue());
...
scoped_ptr<base::ListValue> Create(const std::vector<linked_ptr<SinkInfo> >& sinks) {
  scoped_ptr<base::ListValue> create_results(new base::ListValue());
  create_results->Append(json_schema_compiler::util::CreateValueFromArray(sinks).release());
