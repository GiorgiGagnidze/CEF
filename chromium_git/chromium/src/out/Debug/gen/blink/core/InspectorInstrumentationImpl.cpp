...
InspectorInstrumentationCookie willExecuteScriptImpl(InstrumentingAgents* agents, int scriptId)
{
...
void didExecuteScriptImpl(const InspectorInstrumentationCookie& paramInspectorInstrumentationCookie)
{
...
void didHandleEventImpl(const InspectorInstrumentationCookie& paramInspectorInstrumentationCookie)
{
    ...
    if (InspectorDOMDebuggerAgent* agent = paramInspectorInstrumentationCookie.instrumentingAgents()->inspectorDOMDebuggerAgent())
        agent->didHandleEvent();
}
...
void willEvaluateScriptImpl(InstrumentingAgents* agents)
{
    if (InspectorDOMDebuggerAgent* agent = agents->inspectorDOMDebuggerAgent())
        agent->willEvaluateScript();
}
...
void didUpdateLayoutImpl(InstrumentingAgents* agents)
{
    if (InspectorPageAgent* agent = agents->inspectorPageAgent())
...
void didBlockRequestImpl(InstrumentingAgents* agents, LocalFrame* paramLocalFrame, const ResourceRequest& paramResourceRequest, DocumentLoader* paramDocumentLoader, const FetchInitiatorInfo& paramFetchInitiatorInfo, ResourceRequestBlockedReason paramResourceRequestBlockedReason)
{
    if (InspectorResourceAgent* agent = agents->inspectorResourceAgent())
...
void willSendRequestImpl(InstrumentingAgents* agents, LocalFrame* paramLocalFrame, unsigned long identifier, DocumentLoader* paramDocumentLoader, ResourceRequest& paramResourceRequest, const ResourceResponse& redirectResponse, const FetchInitiatorInfo& paramFetchInitiatorInfo)
{
    if (InspectorResourceAgent* agent = agents->inspectorResourceAgent())
...
void willLoadXHRImpl(InstrumentingAgents* agents, XMLHttpRequest* xhr, ThreadableLoaderClient* client, const AtomicString& method, const KURL& url, bool async, PassRefPtr<EncodedFormData> prpParamEncodedFormData, const HTTPHeaderMap& headers, bool includeCredentials)
{
...
void willStartFetchImpl(InstrumentingAgents* agents, ThreadableLoaderClient* paramThreadableLoaderClient)
{
    if (InspectorResourceAgent* agent = agents->inspectorResourceAgent())
        agent->willStartFetch(paramThreadableLoaderClient);
}

void didFailFetchImpl(InstrumentingAgents* agents, ThreadableLoaderClient* paramThreadableLoaderClient)
{
    if (InspectorResourceAgent* agent = agents->inspectorResourceAgent())
        agent->didFailFetch(paramThreadableLoaderClient);
}

void didFinishFetchImpl(InstrumentingAgents* agents, ExecutionContext* paramExecutionContext, ThreadableLoaderClient* paramThreadableLoaderClient, const AtomicString& method, const String& url)
{
    if (InspectorResourceAgent* agent = agents->inspectorResourceAgent())
...
void didCreateAnimationImpl(InstrumentingAgents* agents, unsigned paramunsigned)
{
    if (InspectorAnimationAgent* agent = agents->inspectorAnimationAgent())
        agent->didCreateAnimation(paramunsigned);
}

void animationPlayStateChangedImpl(InstrumentingAgents* agents, Animation* paramAnimation, Animation::AnimationPlayState oldPlayState, Animation::AnimationPlayState newPlayState)
{
    if (InspectorAnimationAgent* agent = agents->inspectorAnimationAgent())
        agent->animationPlayStateChanged(paramAnimation, oldPlayState, newPlayState);
}

void windowCreatedImpl(InstrumentingAgents* agents, LocalFrame* created)
{
    if (InspectorPageAgent* agent = agents->inspectorPageAgent())
