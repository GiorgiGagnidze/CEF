...
struct ServiceWorkerRegistrationBuilder
 ...
 public:
  ...
  scoped_refptr<ServiceWorkerRegistrationBuilder<MASK>>
  set_force_update_on_page_load(bool force_update_on_page_load) {
    dict_->Set("forceUpdateOnPageLoad", CreateValue(force_update_on_page_load));
    return this;
  }
...
namespace service_worker {
template<int MASK>
struct DebugOnStartUpdatedParamsBuilder
    : base::RefCounted<DebugOnStartUpdatedParamsBuilder<MASK>> {
 public:
  enum {
    kAllSet = 0,
    kDebugOnStart = 1 << 0,
    kNoneSet = kDebugOnStart
  };

  scoped_refptr<DebugOnStartUpdatedParamsBuilder<MASK & ~kDebugOnStart>>
  set_debug_on_start(bool debug_on_start) {
    static_assert(MASK & kDebugOnStart, "already set");
    dict_->Set("debugOnStart", CreateValue(debug_on_start));
    return ThisAs<DebugOnStartUpdatedParamsBuilder<MASK & ~kDebugOnStart>>();
  }

  static scoped_refptr<DebugOnStartUpdatedParamsBuilder<kNoneSet>> Create() {
    return new DebugOnStartUpdatedParamsBuilder<kNoneSet>();
  }
 ...
 private:
  friend struct DebugOnStartUpdatedParamsBuilder<0>;
  friend class base::RefCounted<DebugOnStartUpdatedParamsBuilder<MASK>>;
  ~DebugOnStartUpdatedParamsBuilder() {}

  DebugOnStartUpdatedParamsBuilder() : dict_(new base::DictionaryValue()) {
  }
...
typedef DebugOnStartUpdatedParamsBuilder<0> DebugOnStartUpdatedParams;
...
struct GPUInfoBuilder
 ...
 public:
  ...
  set_aux_attributes(scoped_ptr<base::DictionaryValue> aux_attributes) {
    dict_->Set("auxAttributes", CreateValue(aux_attributes));
    return this;
  }

  scoped_refptr<GPUInfoBuilder<MASK>>
  set_feature_status(scoped_ptr<base::DictionaryValue> feature_status) {
...
class DevToolsProtocolDispatcher {
 ...
 private:
  ...
  bool OnServiceWorkerInspectWorker(
      DevToolsCommandId command_id,
      scoped_ptr<base::DictionaryValue> params);
  bool OnServiceWorkerSetDebugOnStart(
      DevToolsCommandId command_id,
      scoped_ptr<base::DictionaryValue> params);
