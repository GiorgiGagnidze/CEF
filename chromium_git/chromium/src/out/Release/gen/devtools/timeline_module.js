...
return;var asyncEventsByGroup=new Map();this._processAsyncEvents(asyncEventsByGroup,browserMain.asyncEvents());this._mergeAsyncEvents(this._mainThreadAsyncEventsByGroup,asyncEventsByGroup);},_buildTimelineRecords:function()
...
break;case recordTypes.LatencyInfoFlow:if(typeof event.args["layerTreeId"]!=="undefined"&&event.args["layerTreeId"]!==this._inspectedTargetLayerTreeId)
break;this._knownInputEvents.add(event.bind_id);break;case recordTypes.Animation:break;}
...
return null;var data=lastStep.args["data"];asyncEvent.causedFrame=!!(data&&data["INPUT_EVENT_LATENCY_RENDERER_SWAP_COMPONENT"]);if(asyncEvent.name===WebInspector.TimelineModel.RecordType.InputLatencyMouseMove){if(!this._knownInputEvents.has(lastStep.id))
return null;if(!asyncEvent.causedFrame)
return null;}
return groups.input;}
...
{for(var group of source.keys()){var events=target.get(group)||[];events=events.mergeOrdered(source.get(group)||[],WebInspector.TracingModel.Event.compareStartAndEndTime);target.set(group,events);}},loadFromFile:function(file,progress)
...
this._processAnimations(animations);var range=new WebInspector.SegmentedRange();range.append(new WebInspector.Segment(timelineModel.minimumRecordTime(),timelineModel.maximumRecordTime(),WebInspector.TimelineIRModel.Phases.Idle));range.appendRange(this._drags);range.appendRange(this._cssAnimations);range.appendRange(this._scrolls);range.appendRange(this._responses);this._segments=range.segments();},_processInputLatencies:function(events)
...
touchStart=event;firstTouchMove=null;break;case eventTypes.TouchCancel:touchStart=null;break;case eventTypes.TouchMove:if(firstTouchMove||!touchStart)
break;firstTouchMove=event;this._responses.append(new WebInspector.Segment(touchStart.startTime,firstTouchMove.endTime,phases.Response));break;case eventTypes.TouchEnd:if(!touchStart)
break;this._drags.append(new WebInspector.Segment(firstTouchMove?firstTouchMove.endTime:touchStart.startTime,event.endTime,phases.Drag));touchStart=null;break;case eventTypes.MouseDown:mouseDown=event;mouseMove=null;break;case eventTypes.MouseMove:if(mouseDown&&!mouseMove&&mouseDown.startTime+thresholdsMs.mouse>event.startTime){this._responses.append(this._segmentForEvent(mouseDown,phases.Response));this._responses.append(this._segmentForEvent(event,phases.Response));}else if(mouseDown){if(mouseMove&&canMerge(thresholdsMs.mouse,mouseMove,event))
this._drags.append(new WebInspector.Segment(mouseMove.endTime,event.startTime,phases.Drag));this._drags.append(this._segmentForEvent(event,phases.Drag));}
...
{var thresholdsMs=WebInspector.TimelineIRModel._mergeThresholdsMs;this._segments=[];this._drags=new WebInspector.SegmentedRange(merge.bind(null,0));this._cssAnimations=new WebInspector.SegmentedRange(merge.bind(null,WebInspector.TimelineIRModel._mergeThresholdsMs.animation));this._responses=new WebInspector.SegmentedRange(merge.bind(null,0));this._scrolls=new WebInspector.SegmentedRange(merge.bind(null,0));function merge(threshold,first,second)
...
return(eventName.substr(prefix.length));}};WebInspector.SegmentedRange=function(mergeCallback)
...
{return this.begin<that.end&&that.begin<this.end;}};WebInspector.SegmentedRange.prototype={append:function(newSegment)
...
return null;merged.begin=first.begin;merged.end=Math.max(first.end,second.end);return merged;}};WebInspector.TimelineJSProfileProcessor={};WebInspector.TimelineJSProfileProcessor.generateTracingEventsFromCpuProfile=function(jsProfile,thread)
...
continue;var stackTrace=nodeToStackMap.get(node);if(!stackTrace){stackTrace=(new Array(node.depth+1));nodeToStackMap.set(node,stackTrace);for(var j=0;node.parent;node=node.parent)
stackTrace[j++]=(node);}
var jsSampleEvent=new WebInspector.TracingModel.Event(WebInspector.TracingModel.DevToolsTimelineEventCategory,WebInspector.TimelineModel.RecordType.JSSample,WebInspector.TracingModel.Phase.Instant,timestamps[i],thread);jsSampleEvent.args["data"]={stackTrace:stackTrace};jsEvents.push(jsSampleEvent);}
return jsEvents;}
...
var minFrameDurationMs=currentSamplingIntervalMs/2;for(var k=0;k<depth;++k)
jsFramesStack[k].setEndTime(time);for(var k=depth;k<jsFramesStack.length;++k)
...
{var recordTypes=WebInspector.TimelineModel.RecordType;var stackTrace;if(e.name===recordTypes.JSSample){var eventData=e.args["data"]||e.args["beginData"];stackTrace=(eventData&&eventData["stackTrace"]);}else{stackTrace=(jsFramesStack.map(frameEvent=>frameEvent.args["data"]).reverse());}
...
{WebInspector.TimelineFlameChartDataProviderBase.call(this,model);this._frameModel=frameModel;this._irModel=irModel;this._consoleColorGenerator=new WebInspector.FlameChart.ColorGenerator({min:30,max:55,count:5},{min:70,max:100,count:6},50,0.7);}
...
{WebInspector.TimelineFlameChartDataProviderBase.prototype.reset.call(this);this._entryData=[];this._entryTypeByLevel=[];this._entryIndexToTitle=[];this._markers=[];this._asyncColorByCategory={};this._blackboxingURLCache=new Map();},timelineData:function()
{if(this._timelineData)
return this._timelineData;this._timelineData=new WebInspector.FlameChart.TimelineData([],[],[]);this._flowEventIndexById={};this._minimumBoundary=this._model.minimumRecordTime();this._timeSpan=this._model.isEmpty()?1000:this._model.maximumRecordTime()-this._minimumBoundary;this._currentLevel=0;this._appendFrameBars(this._frameModel.frames());this._appendInteractionRecords();this._appendThreadTimelineData(WebInspector.UIString("Main Thread"),this._model.mainThreadEvents(),this._model.mainThreadAsyncEvents());if(Runtime.experiments.isEnabled("gpuTimeline"))
...
{var firstLevel=this._currentLevel;this._appendAsyncEvents(asyncEvents);this._appendSyncEvents(threadTitle,syncEvents);if(this._currentLevel!==firstLevel)
...
if(headerName){this._appendHeaderRecord(headerName);headerName=null;}
...
return(this._blackboxingURLCache.get(url));var result=WebInspector.BlackboxSupport.isBlackboxedURL(url);this._blackboxingURLCache.set(url,result);return result;},_appendAsyncEvents:function(asyncEvents)
...
continue;if(!groupHeaderAppended){this._appendHeaderRecord(header);groupHeaderAppended=true;}
...
if(type===WebInspector.TimelineFlameChartEntryType.Event){var event=(data);if(event.warning)
...
{var type=this._entryType(entryIndex);return type===WebInspector.TimelineFlameChartEntryType.Frame||type===WebInspector.TimelineFlameChartEntryType.Event&&!!(this._entryData[entryIndex]).warning;},_appendHeaderRecord:function(title)
...
{var groupNode=new WebInspector.TimelineTreeView.ProfileTreeNode();groupNode.id=id;groupNode.selfTime=0;groupNode.totalTime=0;groupNode.children=new Map();this._groupNodes.set(id,groupNode);var categories=WebInspector.TimelineUIUtils.categories();switch(this._groupBySetting.get()){case WebInspector.AggregatedTimelineTreeView.GroupBy.Category:var category=categories[id]||categories["other"];groupNode.name=category.title;groupNode.color=category.color;break;case WebInspector.AggregatedTimelineTreeView.GroupBy.Domain:case WebInspector.AggregatedTimelineTreeView.GroupBy.Subdomain:case WebInspector.AggregatedTimelineTreeView.GroupBy.URL:groupNode.name=id||WebInspector.UIString("unattributed");groupNode.color=id?WebInspector.TimelineUIUtils.eventColor(event):categories["other"].color;break;}
...
{var eventStyles=WebInspector.TimelineUIUtils._initEventStyles();if(event.hasCategory(WebInspector.TimelineModel.Category.Console)||event.hasCategory(WebInspector.TimelineModel.Category.UserTiming)||event.hasCategory(WebInspector.TimelineModel.Category.LatencyInfo))
...
this._irModel=new WebInspector.TimelineIRModel();this._model.addEventListener(WebInspector.TimelineModel.Events.RecordingStarted,this._onRecordingStarted,this);this._model.addEventListener(WebInspector.TimelineModel.Events.RecordingStopped,this._onRecordingStopped,this);this._model.addEventListener(WebInspector.TimelineModel.Events.RecordsCleared,this._onRecordsCleared,this);this._model.addEventListener(WebInspector.TimelineModel.Events.BufferUsage,this._onTracingBufferUsage,this);this._model.addEventListener(WebInspector.TimelineModel.Events.RetrieveEventsProgress,this._onRetrieveEventsProgress,this);if(Runtime.experiments.isEnabled("cpuThrottling"))
...
{modeView.setWindowTimes(this.windowStartTime(),this.windowEndTime());modeView.refreshRecords();this._stackView.appendView(modeView.view(),"timelinePanelTimelineStackSplitViewState",undefined,112);this._currentViews.push(modeView);},_removeAllModeViews:function()
...
{this._setState(WebInspector.TimelinePanel.State.Idle);this._frameModel.reset();this._frameModel.addTraceEvents(this._model.target(),this._model.inspectedTargetEvents(),this._model.sessionId()||"");if(this._irModel)
this._irModel.populate(this._model);this._overviewPane.reset();this._overviewPane.setBounds(this._model.minimumRecordTime(),this._model.maximumRecordTime());this._setAutoWindowTimes();this._refreshViews();for(var i=0;i<this._overviewControls.length;++i)
...
{}
WebInspector.TimelineModeView.prototype={view:function(){},dispose:function(){},reset:function(){},refreshRecords:function(){},highlightSearchResult:function(record,regex,selectRecord){},setWindowTimes:function(startTime,endTime){},setSelection:function(selection){},highlightEvent:function(event){}}
