...
CORE_EXPORT InspectorInstrumentationCookie willExecuteScriptImpl(InstrumentingAgents*, int);

inline InspectorInstrumentationCookie willExecuteScript(ExecutionContext* paramExecutionContext, int scriptId)
{   
    FAST_RETURN_IF_NO_FRONTENDS(InspectorInstrumentationCookie());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        return willExecuteScriptImpl(agents, scriptId);
    return InspectorInstrumentationCookie();
}

CORE_EXPORT void didExecuteScriptImpl(const InspectorInstrumentationCookie&);

inline void didExecuteScript(const InspectorInstrumentationCookie& paramInspectorInstrumentationCookie)
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (paramInspectorInstrumentationCookie.isValid())
        didExecuteScriptImpl(paramInspectorInstrumentationCookie);
}
...
CORE_EXPORT void willEvaluateScriptImpl(InstrumentingAgents*);

inline void willEvaluateScript(ExecutionContext* paramExecutionContext)
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        willEvaluateScriptImpl(agents);
}
...
CORE_EXPORT void didUpdateLayoutImpl(InstrumentingAgents*);

inline void didUpdateLayout(LocalFrame* paramLocalFrame)
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramLocalFrame))
        didUpdateLayoutImpl(agents);
}
...
CORE_EXPORT void didBlockRequestImpl(InstrumentingAgents*, LocalFrame*, const ResourceRequest&, DocumentLoader*, const FetchInitiatorInfo&, ResourceRequestBlockedReason);

inline void didBlockRequest(LocalFrame* paramLocalFrame, const ResourceRequest& paramResourceRequest, DocumentLoader* paramDocumentLoader, const FetchInitiatorInfo& paramFetchInitiatorInfo, ResourceRequestBlockedReason paramResourceRequestBlockedReason)
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramLocalFrame))
        didBlockRequestImpl(agents, paramLocalFrame, paramResourceRequest, paramDocumentLoader, paramFetchInitiatorInfo, paramResourceRequestBlockedReason);
}

CORE_EXPORT void willSendRequestImpl(InstrumentingAgents*, LocalFrame*, unsigned long, DocumentLoader*, ResourceRequest&, const ResourceResponse&, const FetchInitiatorInfo&);
...
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramLocalFrame))
        willSendRequestImpl(agents, paramLocalFrame, identifier, paramDocumentLoader, paramResourceRequest, redirectResponse, paramFetchInitiatorInfo);
}
...
CORE_EXPORT void willLoadXHRImpl(InstrumentingAgents*, XMLHttpRequest*, ThreadableLoaderClient*, const AtomicString&, const KURL&, bool, PassRefPtr<EncodedFormData>, const HTTPHeaderMap&, bool);

inline void willLoadXHR(ExecutionContext* paramExecutionContext, XMLHttpRequest* xhr, ThreadableLoaderClient* client, const AtomicString& method, const KURL& url, bool async, PassRefPtr<EncodedFormData> prpParamEncodedFormData, const HTTPHeaderMap& headers, bool includeCredentials)
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        willLoadXHRImpl(agents, xhr, client, method, url, async, prpParamEncodedFormData, headers, includeCredentials);
}
...
CORE_EXPORT void willStartFetchImpl(InstrumentingAgents*, ThreadableLoaderClient*);

inline void willStartFetch(ExecutionContext* paramExecutionContext, ThreadableLoaderClient* paramThreadableLoaderClient)
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        willStartFetchImpl(agents, paramThreadableLoaderClient);
}

CORE_EXPORT void didFailFetchImpl(InstrumentingAgents*, ThreadableLoaderClient*);

inline void didFailFetch(ExecutionContext* paramExecutionContext, ThreadableLoaderClient* paramThreadableLoaderClient)
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        didFailFetchImpl(agents, paramThreadableLoaderClient);
}

CORE_EXPORT void didFinishFetchImpl(InstrumentingAgents*, ExecutionContext*, ThreadableLoaderClient*, const AtomicString&, const String&);

inline void didFinishFetch(ExecutionContext* paramExecutionContext, ThreadableLoaderClient* paramThreadableLoaderClient, const AtomicString& method, const String& url)
{   
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramExecutionContext))
        didFinishFetchImpl(agents, paramExecutionContext, paramThreadableLoaderClient, method, url);
}
...
CORE_EXPORT void didCreateAnimationImpl(InstrumentingAgents*, unsigned);

inline void didCreateAnimation(Document* paramDocument, unsigned paramunsigned)
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramDocument))
        didCreateAnimationImpl(agents, paramunsigned);
}

CORE_EXPORT void animationPlayStateChangedImpl(InstrumentingAgents*, Animation*, Animation::AnimationPlayState, Animation::AnimationPlayState);
...
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(paramDocument))
        animationPlayStateChangedImpl(agents, paramAnimation, oldPlayState, newPlayState);
}

CORE_EXPORT void windowCreatedImpl(InstrumentingAgents*, LocalFrame*);

inline void windowCreated(LocalFrame* opener, LocalFrame* created)
{   
    FAST_RETURN_IF_NO_FRONTENDS(void());
    if (InstrumentingAgents* agents = instrumentingAgentsFor(opener))
        windowCreatedImpl(agents, created);
}
