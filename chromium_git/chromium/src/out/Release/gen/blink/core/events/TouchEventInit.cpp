// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file has been auto-generated by code_generator_v8.py. DO NOT MODIFY!

#include "TouchEventInit.h"


namespace blink {

TouchEventInit::TouchEventInit()
{
    setChangedTouches(WillBeHeapVector<RefPtrWillBeMember<Touch>>());
    setTargetTouches(WillBeHeapVector<RefPtrWillBeMember<Touch>>());
    setTouches(WillBeHeapVector<RefPtrWillBeMember<Touch>>());
}

TouchEventInit::~TouchEventInit()
{
}

DEFINE_TRACE(TouchEventInit)
{
    visitor->trace(m_changedTouches);
    visitor->trace(m_targetTouches);
    visitor->trace(m_touches);
    EventModifierInit::trace(visitor);
}

} // namespace blink
