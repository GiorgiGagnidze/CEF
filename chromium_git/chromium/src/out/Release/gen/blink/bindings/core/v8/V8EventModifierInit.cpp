...
void V8EventModifierInit::toImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8Value, EventModifierInit& impl, ExceptionState& exceptionState)
{
    ...
    {
        v8::Local<v8::Value> modifierOSValue;
        if (!v8Object->Get(isolate->GetCurrentContext(), v8String(isolate, "modifierOS")).ToLocal(&modifierOSValue)) {
            exceptionState.rethrowV8Exception(block.Exception());
            return;
        }
        if (modifierOSValue.IsEmpty() || modifierOSValue->IsUndefined()) {
            // Do nothing.
        } else {
            bool modifierOS = toBoolean(isolate, modifierOSValue, exceptionState);
            if (exceptionState.hadException())
                return;
            impl.setModifierOS(modifierOS);
        }
    }
...
bool toV8EventModifierInit(const EventModifierInit& impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate)
{
    ...
    if (impl.hasModifierOS()) {
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "modifierOS"), v8Boolean(impl.modifierOS(), isolate))))
            return false;
    } else {
        if (!v8CallBoolean(dictionary->CreateDataProperty(isolate->GetCurrentContext(), v8String(isolate, "modifierOS"), v8Boolean(false, isolate))))
            return false;
    }
