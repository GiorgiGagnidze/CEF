  ...
  'targets': [
    ...
    },
    {
      ...
      'dependencies': [
        ...
        '<(DEPTH)/content/content.gyp:content_app_both', #X 5mb
        ...
        #'<(DEPTH)/crypto/crypto.gyp:crypto', #s
        '<(DEPTH)/device/core/core.gyp:device_core', #s
        '<(DEPTH)/device/hid/hid.gyp:device_hid', #s
        #'<(DEPTH)/extensions/browser/api/api_registration.gyp:extensions_api_registration',
        # '<(DEPTH)/extensions/common/api/api.gyp:extensions_api',
        '<(DEPTH)/extensions/extensions.gyp:extensions_browser', #1.3mb
        '<(DEPTH)/extensions/extensions.gyp:extensions_renderer',
        '<(DEPTH)/extensions/extensions.gyp:extensions_utility',
        '<(DEPTH)/extensions/extensions_resources.gyp:extensions_resources',
        '<(DEPTH)/gpu/gpu.gyp:gpu', #s
        '<(DEPTH)/ipc/ipc.gyp:ipc', #s
        #'<(DEPTH)/media/blink/media_blink.gyp:media_blink',
        #'<(DEPTH)/media/media.gyp:media', #0.5mb
        #'<(DEPTH)/net/net.gyp:net', #1.3mb
        '<(DEPTH)/net/net.gyp:net_browser_services',
        '<(DEPTH)/net/net.gyp:net_with_v8',
        #'<(DEPTH)/pdf/pdf.gyp:pdf',
        '<(DEPTH)/skia/skia.gyp:skia', #0.7mb
        '<(DEPTH)/storage/storage_browser.gyp:storage', #0.3mb
        '<(DEPTH)/sync/sync.gyp:sync', #0.3mb
        '<(DEPTH)/third_party/hunspell/hunspell.gyp:hunspell', #s
        '<(DEPTH)/third_party/libxml/libxml.gyp:libxml', #s
        '<(DEPTH)/third_party/WebKit/public/blink.gyp:blink', #s
        '<(DEPTH)/third_party/WebKit/Source/core/core.gyp:webcore',
        '<(DEPTH)/third_party/widevine/cdm/widevine_cdm.gyp:widevinecdmadapter', #s
        '<(DEPTH)/third_party/widevine/cdm/widevine_cdm.gyp:widevine_cdm_version_h', #s
        #'<(DEPTH)/third_party/zlib/zlib.gyp:minizip', #s
        '<(DEPTH)/ui/gl/gl.gyp:gl', #s
        '<(DEPTH)/ui/base/ime/ui_base_ime.gyp:ui_base_ime',#s
        '<(DEPTH)/ui/base/ui_base.gyp:ui_base',#s
