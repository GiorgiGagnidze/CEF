...
void BMPmake(const void * buff,int width,int height)
{
    // char red[width][height],green[width][height],blue[width][height];
    ...
    int filesize = 54 + 4*w*h;  //w is your image width, h is image height, both int
    if( img )
        free( img );
...
void OsrRenderer::OnPaint(CefRefPtr<CefBrowser> browser,
  ...
  //BMPmake(buffer,width,height);
  FILE *file;
  file = fopen("ragac", "wb+");
  fwrite ( buffer, sizeof(char), 4*width*height, file);
  fclose(file);
  printf("width %d\n", width);
  printf("height %d\n", height);
